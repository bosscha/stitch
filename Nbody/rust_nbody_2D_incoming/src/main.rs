...
fn sample_king_nd(n_stars: usize, r_c: f32, r_t: f32) -> Vec<Vector3> {
...
async fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..N_STARS {
        for j in 0..N_STARS {
            ...
            let dz = pos[j][2] - pos[i][2];
            let dist_sq = dx*dx + dy*dy + dz*dz;
            let force_mag = G * mass[j] / (dist_sq + SOFTENING * SOFTENING);
    ...
    let mut particles = vec![Particle { pos_mass: [0.0; 4], vel: [0.0; 4], acc: [0.0; 4] }; N_STARS];
    for i in 0..N_STARS {
        particles[i].pos_mass = [pos[i][0], pos[i][1], pos[i][2], mass[i]];
        particles[i].vel = [vel[i][0], vel[i][1], vel[i][2], 0.0];
        particles[i].acc = [initial_acc[i][0], initial_acc[i][1], initial_acc[i][2], 0.0];
    }
    ...
    for batch in 0..num_batches {
        ...
        if let Ok(Ok(())) = rx.recv() {
            ...
            let mut escaped = 0;
            for p in result {
                let dx = p.pos_mass[0] - center[0];
                let dy = p.pos_mass[1] - center[1];
                let dz = p.pos_mass[2] - center[2];
                if (dx*dx + dy*dy + dz*dz).sqrt() > r80_initial {
                    escaped += 1;
                }
            }
            ...
            if let Some(ref mut client) = db_client {
                ...
                for i in 0..N_STARS {
                    let pos_vec = vec![result[i].pos_mass[0] as f64, result[i].pos_mass[1] as f64, result[i].pos_mass[2] as f64];
                    let vel_vec = vec![result[i].vel[0] as f64, result[i].vel[1] as f64, result[i].vel[2] as f64];
                    let _ = trans.execute("
    ...
    // Plots can be generated here using time_steps_list and escaped_counts.
