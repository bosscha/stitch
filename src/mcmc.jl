...
function theta(p::GaiaClustering.abc)
    ...
    e = rand(peps)
    pe = pdf(peps, e)
    n = trunc(Int, rand(pminnei))
    pn = pdf(pminnei, n)
    c = trunc(Int, rand(pmincl))
    pc = pdf(pmincl, c)
        ...
        while loopAgain
            ...
            if qres > minimumQ && nstars >= minstars
                ...
                if α > rand()
                    ...
                    if nchain > nburn && !burndone
                        println("### burn-in done...")
                        nchain = 0
                        burndone = true
                    end
                    if nchain > niter
    ...
    let
        ...
        while loopAgain
            ...
            if iter > maxiter
                ...
                if burndone
                    FLAG = 3
                end    ## nchain not reached before maxiter but burnin done...
                if !burndone
                    FLAG = 4
                end    ## burn-in not performed
                return (mci, iter, FLAG)
            end
        end
